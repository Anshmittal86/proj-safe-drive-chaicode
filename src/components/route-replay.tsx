...
import Svg, { Path, Circle, G, Rect, SvgProps, Text as SvgText } from 'react-native-svg';
import { useTheme } from '../hooks/use-theme';
import { RoutePoint, DriveEvent } from '../services/storage-service';
...
export function RouteReplay({ path, timeline, height = 240 }: RouteReplayProps) {
  ...
  const project = (latitude: number, longitude: number) => {
    ...
    return { x: Math.max(8, Math.min(mapWidth - 8, x)), y: Math.max(8, Math.min(mapHeight - 8, y)) };
  };
  ...
  const getEventMarkerColor = (type: DriveEvent['type']) => {
    switch (type) {
      case 'phone_handling': return theme.error;
      case 'harsh_brake': return '#F59E0B'; // Amber
      case 'harsh_accel': return '#2563EB'; // Blue
      case 'sharp_turn': return '#8B5CF6'; // Purple
      case 'aggressive_steer': return '#EC4899'; // Pink
  ...
  const getEventLetter = (type: DriveEvent['type']) => {
    switch (type) {
      case 'phone_handling': return 'P';
      case 'harsh_brake': return 'B';
      case 'harsh_accel': return 'A';
      case 'sharp_turn': return 'T';
      case 'aggressive_steer': return 'S';
      default: return '!';
    }
  };
  ...
  return (
    <View style={[styles.container, { borderColor: theme.border }]}>
      <Pressable onPress={() => setSelectedEvent(null)} style={{ width: mapWidth, height: mapHeight }}>
        <Svg width={mapWidth} height={mapHeight}>
          ...
          {startCoord ? (
            <G>
              ...
              <Circle cx={startCoord.x} cy={startCoord.y} r="10" fill="transparent" stroke={theme.success} strokeWidth="1.5" />
            </G>
          ) : null}
          ...
          {endCoord ? (
            <G>
              ...
              <Circle cx={endCoord.x} cy={endCoord.y} r="10" fill="transparent" stroke="#EF4444" strokeWidth="1.5" />
            </G>
          ) : null}
        ...
        {selectedEvent && (
          ...
          >
            ...
            <ThemedText style={styles.tooltipImpact}>Penalty: {selectedEvent.scoreImpact} pts</ThemedText>
      ...
      <View style={styles.legendContainer}>
        <View style={styles.legendRow}>
          <View style={styles.badge}><ThemedText style={styles.badgeText}>B</ThemedText></View>
          <ThemedText type="small" style={styles.legendText}>Braking</ThemedText>
        </View>
        <View style={styles.legendRow}>
          <View style={[styles.badge, { backgroundColor: '#2563EB' }]}><ThemedText style={styles.badgeText}>A</ThemedText></View>
          <ThemedText type="small" style={styles.legendText}>Accel</ThemedText>
        </View>
        <View style={styles.legendRow}>
          <View style={[styles.badge, { backgroundColor: '#8B5CF6' }]}><ThemedText style={styles.badgeText}>T</ThemedText></View>
          <ThemedText type="small" style={styles.legendText}>Turn</ThemedText>
        </View>
        <View style={styles.legendRow}>
          <View style={[styles.badge, { backgroundColor: '#EF4444' }]}><ThemedText style={styles.badgeText}>P</ThemedText></View>
          <ThemedText type="small" style={styles.legendText}>Phone</ThemedText>
        </View>
      </View>
    </View>
  );
